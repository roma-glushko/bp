...
<div class="space-y-6">
  ...
  <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 print:hidden">
    <div class="grid grid-cols-2 gap-4 mb-4">
      <div>
        <label for="report-from" class="block text-xs text-gray-500 dark:text-gray-400 mb-1">From</label>
        <input
          id="report-from"
          type="date"
          bind:value={fromDate}
          class="w-full px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 dark:bg-gray-700 dark:text-gray-100"
        />
      </div>
      <div>
        <label for="report-to" class="block text-xs text-gray-500 dark:text-gray-400 mb-1">To</label>
        <input
          id="report-to"
          type="date"
          bind:value={toDate}
    ...
    <fieldset>
      <legend class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Include sections</legend>
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {#each [
          ...
          <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
            <input type="checkbox" bind:checked={sections[key]} class="rounded border-gray-300 dark:border-gray-600" />
            {label}
          </label>
        {/each}
      </div>
  ...
  {#if report}
    <div class="report-preview bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 print:border-0 print:p-0 print:rounded-none">
      <div class="text-center mb-8">
        <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100">Blood Pressure Report</h2>
        {#if report.patient}
          <p class="text-gray-700 dark:text-gray-300 mt-1">Patient: {report.patient}</p>
        {/if}
        {#if report.device}
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Measured By: {report.device}</p>
        {/if}
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Period: {report.from} — {report.to}</p>
        <p class="text-sm text-gray-500 dark:text-gray-400">Generated: {report.generated}</p>
      </div>

      {#if report.summary}
        <section class="mb-8">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3 border-b border-gray-200 dark:border-gray-700 pb-1">1. Summary</h3>
          <div class="grid grid-cols-3 gap-4 mb-4">
            <div>
              <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">Days Measured</p>
              <p class="text-xl font-semibold text-gray-900 dark:text-gray-100">{report.summary.days_measured}</p>
            </div>
            <div>
              <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">Sessions</p>
              <p class="text-xl font-semibold text-gray-900 dark:text-gray-100">{report.summary.session_count}</p>
            </div>
            <div>
              <p class="text-xs text-gray-500 dark:text-gray-400 uppercase">Readings</p>
              <p class="text-xl font-semibold text-gray-900 dark:text-gray-100">{report.summary.reading_count}</p>
            </div>
          </div>

          <table class="w-full text-sm mb-4">
            <thead>
              <tr class="text-xs text-gray-400 dark:text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700">
          ...
          <div class="grid grid-cols-2 gap-4 text-sm text-gray-700 dark:text-gray-300">
            <div>
              <p class="text-xs text-gray-500 dark:text-gray-400 uppercase mb-1">Blood Pressure Range</p>
            ...
            {#if report.summary.highest_pulse}
              <div>
                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase mb-1">Pulse Range</p>
      ...
      {#if report.month_averages?.length > 0}
        <section class="mb-8">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3 border-b border-gray-200 dark:border-gray-700 pb-1">2. Monthly Averages</h3>
      ...
      {#if report.week_averages?.length > 0}
        <section class="mb-8">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3 border-b border-gray-200 dark:border-gray-700 pb-1">3. Weekly Averages</h3>
          <table class="w-full text-sm">
            <thead>
              <tr class="text-xs text-gray-400 dark:text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700">
      ...
      {#if report.day_averages?.length > 0}
        <section class="mb-8">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3 border-b border-gray-200 dark:border-gray-700 pb-1">4. Daily Averages</h3>
          <table class="w-full text-sm">
            <thead>
              <tr class="text-xs text-gray-400 dark:text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700">
      ...
      {#if report.sessions?.length > 0}
        <section class="mb-8">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3 border-b border-gray-200 dark:border-gray-700 pb-1">5. Session Averages</h3>
          <table class="w-full text-sm">
            <thead>
              <tr class="text-xs text-gray-400 dark:text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700">
                <th class="text-left py-1 font-medium">Date</th>
      ...
      {#if report.readings?.length > 0}
        <section class="mb-8">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3 border-b border-gray-200 dark:border-gray-700 pb-1">6. Raw Measurements</h3>
          <table class="w-full text-sm">
            <thead>
              <tr class="text-xs text-gray-400 dark:text-gray-500 uppercase border-b border-gray-200 dark:border-gray-700">
                <th class="text-left py-1 font-medium">Date</th>
      ...
      <footer class="text-xs text-gray-400 dark:text-gray-500 text-center border-t border-gray-200 dark:border-gray-700 pt-4 mt-8">
...
<style>
  @media print {
    ...
    :global(.dark) .report-preview { color: black !important; }
    :global(.dark) .report-preview * { color: inherit !important; border-color: #e5e7eb !important; }
  }
</style>
