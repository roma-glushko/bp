...
{:else if !lastSession}
  <div class="text-center py-12">
    <h1 class="text-2xl font-bold text-gray-900 mb-2">Welcome to BP Journal</h1>
    <p class="text-gray-600 mb-6">Start tracking your blood pressure by adding your first reading.</p>
    <a
  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div class="bg-white rounded-lg border border-gray-200 p-4">
        <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">Last Reading</p>
        <p class="text-2xl font-semibold text-gray-900">
          {lastSession.average.avg_systolic} / {lastSession.average.avg_diastolic}
        </p>
        {#if lastSession.average.avg_pulse}
          <p class="text-sm text-gray-500">Pulse {lastSession.average.avg_pulse}</p>
        {/if}
        <div class="mt-1">
      ...
      {#if todaySessions.length > 0}
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-4">
          <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">Today's Average</p>
          <p class="text-2xl font-semibold text-gray-900">{todaySys} / {todayDia}</p>
          <div class="mt-1">
            <BPIndicator {...todayIndicator} />
          </div>
          <p class="text-xs text-gray-400 mt-2">{todaySessions.length} session{todaySessions.length !== 1 ? 's' : ''}</p>
        </div>
      {/if}

      {#if weekAvg}
        <div class="bg-white rounded-lg border border-gray-200 p-4">
          <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">7-Day Average</p>
          <p class="text-2xl font-semibold text-gray-900">{weekAvg.avgSys} / {weekAvg.avgDia}</p>
          {#if weekAvg.avgPulse}
            <p class="text-sm text-gray-500">Pulse {weekAvg.avgPulse}</p>
          {/if}
          <div class="mt-1">
    ...
    {#if todaySessions.length > 0}
      <div>
        <h2 class="text-lg font-semibold text-gray-900 mb-3">Today's Sessions</h2>
        <div class="space-y-2">
          {#each todaySessions as session}
            <div class="bg-white rounded-lg border border-gray-200 p-4 flex items-center justify-between">
              <div class="flex items-center gap-4">
                <div>
                  <p class="text-sm font-medium text-gray-900">
                    ...
                    <span class="text-gray-500 capitalize">· {session.period}</span>
                  </p>
                  <p class="text-lg font-semibold text-gray-900">
                    {session.average.avg_systolic} / {session.average.avg_diastolic}
                    {#if session.average.avg_pulse}
                      <span class="text-sm text-gray-500 font-normal">pulse {session.average.avg_pulse}</span>
                    {/if}
                  </p>
                </div>
              </div>
              <div class="flex items-center gap-3">
                ...
                <span class="text-xs text-gray-400">{session.readings.length} reading{session.readings.length !== 1 ? 's' : ''}</span>
              </div>
            </div>
          {/each}
        </div>
